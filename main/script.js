    ...
    function renderTabVideos(videos) {
        ...
        videos.forEach(v => {
            ...
            const adapted = {
                ...
                found:         true,
                cover:         v.cover || v.img || '',
                japanese_title: v.japanese_title || ''
            };
...
function setSyncGridSlots(slots) {
    ...
    // Check all possible 9 slot configurations
    for (let i = 0; i < 9; i++) {
        ...
        if (i < numSlots) {
            ...
            pane.style.display = 'none'; // Hide instead of remove
            
            // Pause video to prevent background audio and optimize performance
            const data = syncPanesData[paneId];
            if (data && data.video) {
                try { data.video.pause(); } catch(e){}
            }
        }
    }
    ...
    // Clear active pane focus if the active pane was removed/hidden
...
function updateActivePaneIndicator() {
    ...
    if (indicator) {
        if (lastClickedPaneId) {
            ...
            indicator.innerHTML = `🎯 鎖定視窗 #${num} 重播按鍵-起點(${hotkeyStart})-終點(${hotkeyEnd})`;
...
// Global key down listener on main window (use capture phase to intercept reliably)
...
// Forwarded keydown messages from player iframes (supports nested frames via parent traversal)
window.addEventListener('message', (e) => {
    if (e.data && e.data.type === 'keydown') {
        const iframes = document.querySelectorAll('.sync-pane iframe');
        let sendingPaneId = null;
        for (let iframe of iframes) {
            ...
            try {
                ...
                // Cross-origin boundary might prevent traversal, fall back to exact match
            }
            if (matched || iframe.contentWindow === e.source) {
                sendingPaneId = iframe.closest('.sync-pane').id;
                break;
            }
        }
        const targetPaneId = sendingPaneId || lastClickedPaneId;
        if (targetPaneId) {
            if (sendingPaneId && sendingPaneId !== lastClickedPaneId) {
                setActivePane(sendingPaneId);
            }
            handleGlobalHotkey(e.data.key, targetPaneId);
        }
    }
});

// Periodically check activeElement to detect focus on player iframes
setInterval(() => {
    const active = document.activeElement;
    if (active && active.tagName === 'IFRAME') {
        const pane = active.closest('.sync-pane');
        if (pane && pane.id && pane.id !== lastClickedPaneId) {
...
function handleGlobalHotkey(key, paneId) {
    ...
    if (k === hotkeyStart.toLowerCase() || k === 'z') {
        setLoopStart(paneId);
    } else if (k === hotkeyEnd.toLowerCase() || k === 'x') {
        setLoopEnd(paneId);
    }
}
...
async function addToSyncWatch(url) {
    ...
    // Find first visible empty pane
    let pane = null;
    for (let i = 0; i < paneCount; i++) {
        const p = document.getElementById(`sync-pane-${i}`);
        if (p && p.classList.contains('empty-pane')) {
            pane = p;
            break;
        }
    }
        ...
        <div class="sync-loop-overlay">
            <div class="loop-status-pill">
                <div class="loop-btn-group">
                    <button class="loop-btn btn-set-a" onclick="setLoopStart('${paneId}'); event.stopPropagation();" title="將當前時間設為起點">起點</button>
                    <button class="loop-btn btn-set-b" onclick="setLoopEnd('${paneId}'); event.stopPropagation();" title="將當前時間設為終點" disabled>終點</button>
...
function findActiveVideo(paneId) {
    const pane = document.getElementById(paneId);
    if (!pane) return null;
    const iframe = pane.querySelector('iframe');
    if (!iframe || !iframe.contentWindow) return null;
    return findVideo(iframe.contentWindow);
}

function isVideoAttached(video) {
    if (!video) return false;
    try {
        return video.isConnected || (video.ownerDocument && video.ownerDocument.body && video.ownerDocument.body.contains(video));
    } catch (e) {
        return false;
    }
}

function ensureActiveVideo(paneId) {
    const data = syncPanesData[paneId];
    if (!data) return null;
    if (!data.video || !isVideoAttached(data.video)) {
        const v = findActiveVideo(paneId);
        if (v) data.video = v;
    }
    return data.video;
}
...
function setLoopStart(paneId) {
    ...
    const video = ensureActiveVideo(paneId);
    if (!video) {
        showToast("⚠️ 請先播放影片後再設定！");
        return;
    }
    
    data.start = video.currentTime;
    ...
    btnA.innerHTML = `${formatTime(data.start)}`;
    btnB.disabled = false;
...
function setLoopEnd(paneId) {
    ...
    btnB.innerHTML = `${formatTime(data.end)}`;
...
function startLoopMonitor(paneId) {
    ...
    syncPanesData[paneId].monitor = setInterval(() => {
        const data = syncPanesData[paneId];
        if (!data || !document.getElementById(paneId) || document.getElementById(paneId).classList.contains('empty-pane')) {
            if (data && data.monitor) clearInterval(data.monitor);
            return;
        }
        
        const video = ensureActiveVideo(paneId);
        if (video && data.start !== null && data.end !== null) {
        ...
        if (currentContextMenuSource === 'home' && currentContextMenuVideo) {
            ...
            if(!allVideos.find(v => (v.code || '').toLowerCase().trim() === cleanTarget)) {
                ...
                const videoData = {
                    ...
                    japanese_title: currentContextMenuVideo.japanese_title || "",
...
function initializeApp() {
    ...
    loadData();
    // 啟動後立即載入首頁並背景預載其他線上影音分頁
    if (!homeLoaded) {
        homeLoaded = true;
        homeManager.onHomeActivated();
        setTimeout(() => {
            console.log("JApp: Background preloading remaining sections...");
...
function createVideoCardElement(v, source = 'fav') {
    ...
    card.videoData = v; // Store reference to object for metadata updates
    ...
    let dateHtml = '';
    if (displayDate) {
        dateHtml = `<div class="card-date" style="color: #94a3b8; font-size: 0.75rem; display: flex; align-items: center; gap: 4px;" title="日期">${displayDate}</div>`;
    }
    
    const finalActress = (cleanActress && cleanActress !== '新加入' && cleanActress !== '未知' && cleanActress !== '未知女優') 
        ? cleanActress 
        : '未知女優';
    ...
    card.innerHTML = `
        ...
        <div class="card-content">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 0.4rem;">
                <div class="card-code" style="margin-bottom: 0;">${cleanCode}</div>
                <div class="card-actress" style="font-size: 0.85rem; color: #94a3b8; margin-bottom: 0;">${finalActress.replace(/\*\*/g, '')}</div>
            </div>
            ${(v.japanese_title || v.title) ? `<div class="card-jp-title" style="font-size: 0.85rem; color: #cbd5e1; margin-bottom: 0.6rem; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; word-break: break-word;" title="${(v.japanese_title || v.title).replace(/"/g, '&quot;')}">${v.japanese_title || v.title}</div>` : ''}
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.6rem;">
                <div class="card-type ${getTypeClass(displayType)}" style="margin-bottom: 0;">${displayType}</div>
                ${dateHtml}
            </div>
...
function renderVideos(videos, shouldPopulateFilters = true) {
    ...
    // Dynamically trigger background metadata scraping for rendered cards if actress is unknown
    if (videos && videos.length > 0) {
        triggerAsyncMetadataFetch(videos);
    }
}
...
async function triggerAsyncMetadataFetch(videos) {
    if (!window.pywebview || !window.pywebview.api) return;
    
    let newCodes = [];
    videos.forEach(v => {
        if (v.code && (!v.actress || v.actress === '未知' || v.actress === '未知女優' || v.actress === 'None' || v.actress === '')) {
    ...
    if (newCodes.length > 0) {
        // First: immediately query existing cache to render any already-known data
        try {
            const cachedResults = await window.pywebview.api.get_cached_metadata(newCodes);
            if (cachedResults) {
                applyMetadataToCards(cachedResults, newCodes);
            }
        } catch(e) {}
        
        // Then: trigger background fetch for remaining unknowns
        newCodes.forEach(code => {
            window.pywebview.api.request_actress_async(code).catch(e => console.error(e));
        });
        
        if (!metadataPollTimer) {
            metadataPollTimer = setInterval(pollMetadata, 1500);
        }
    }
}

function applyMetadataToCards(results, codesToPoll) {
    let dbChanged = false;
    for (const code of codesToPoll) {
        const codeLower = code.toLowerCase();
        const codeUpper = code.toUpperCase();
        const metadata = results[codeLower] || results[codeUpper] || results[code];
        if (!metadata) continue;
        
        const act = metadata.actress;
        const jpTitle = metadata.japanese_title;
        const card = document.getElementById('video-card-' + codeUpper);
        
        if (card) {
            if (card.videoData) {
                if (act && act !== '未知' && card.videoData.actress !== act) {
                    card.videoData.actress = act;
                    dbChanged = true;
                }
                if (jpTitle && card.videoData.japanese_title !== jpTitle) {
                    card.videoData.japanese_title = jpTitle;
                    dbChanged = true;
                }
            }
            ...
            if (act && act !== '未知' && act !== '') {
                ...
                if (actressElem && actressElem.textContent !== act) {
                    actressElem.textContent = act.replace(/\*\*/g, '');
                    actressElem.style.color = '#38bdf8';
            ...
            if (jpTitle) {
                const jpTitleElem = card.querySelector('.card-jp-title');
                if (jpTitleElem && jpTitleElem.textContent !== jpTitle) {
                    jpTitleElem.textContent = jpTitle;
                    jpTitleElem.title = jpTitle;
                }
            }
        }
    }
    if (dbChanged && window.pywebview && allVideos) {
        window.pywebview.api.save_videos(allVideos).catch(e => console.error(e));
    }
}
...
async function pollMetadata() {
    ...
    try {
        ...
        if (results) {
            applyMetadataToCards(results, codesToPoll);
        }
    } catch(e) {
