...
PROXY_POOL = urllib3.PoolManager(
    ...
    retries=urllib3.util.Retry(total=3, backoff_factor=0.2, status_forcelist=[500, 502, 503, 504])
)

EXCLUDE_ACTRESS_TERMS = {
    ...
    # 新增常規影片標題雜詞，防止誤判為女優名字
    "超級", "連射", "直播", "色情", "以上", "以下", "極致", "極限", "精采", "精彩", "影片", "播放", "下載", "分享", "觀看",
    "第一", "第二", "第三", "第四", "第五", "特別版", "完整版", "破壞版", "解禁版", "流出版", "流出", "破解", "字幕", "無碼",
    "極上", "至高", "巔峰", "完美", "頂級", "一流", "頂尖", "最強", "最棒", "最好", "最大", "最高", "最深", "最久",
    "狂歡", "派對", "聚會", "集體", "多人群交", "群交", "雙飛", "三飛", "大亂交", "亂交", "輪流", "輪番", "輪流射"
}

def is_valid_actress_name(q):
    if not q:
        return False
    q_clean = q.strip().lower()
    # 長度判定：中文女優名字一般為 2 到 6 個字
    if not re.match(r'^[\u4e00-\u9fa5]{2,6}$', q_clean):
        return False
    # 排除常見排除詞
    if q_clean in EXCLUDE_ACTRESS_TERMS or any(term in q_clean for term in EXCLUDE_ACTRESS_TERMS):
        return False
    return True
...
def get_data_path():
    ...
    if hasattr(sys, 'frozen'):
        ...
        dist_japp_path = os.path.join(base_dir, 'dist', 'JApp')
        if os.path.exists(os.path.join(dist_japp_path, 'Favorites.json')):
            return dist_japp_path
...
def fetch_url(url):
    ...
    # Extract actress name from details page HTML using robust regex (supporting models, actresses, star)
    actress_name = "未知"
    ...
    return True, img_url, actress_name, title
...
class VideoProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if '/javhdporn_proxy' in parsed_path.path:
                    ...
                    if html:
                        ...
                        <script>
                            (function() {{
                                ...
                                window.addEventListener('keydown', (e) => {{ window.parent.postMessage({{type: 'keydown', key: e.key}}, '*'); }}, true);
            ...
            elif '/local_image' in parsed_path.path:
                code_param = qs.get('code', [''])[0]
                fallback_param = qs.get('fallback', [''])[0]
                cache_path = os.path.join(GLOBAL_API.img_cache_dir, code_param) if (GLOBAL_API and code_param) else ""
                ...
                if cache_path and os.path.exists(cache_path):
                    try:
                        ...
                        safe_print(f"[ImgCache] Failed to serve local image for {code_param}: {e}")
                
                if fallback_param:
                    self.send_response(302)
                    self.send_header('Location', fallback_param)
                    self.end_headers()
                    return
                else:
                    self.send_error(404)
                ...
                if url_param:
                    ...
                    if html:
                        html = strip_ads(html)
                        html = html.replace('<head>', '<head><base href="https://missav.ws/"><script>window.open=function(){return null;};window.onbeforeunload=null;window.addEventListener(\'keydown\', (e) => { window.parent.postMessage({type: \'keydown\', key: e.key}, \'*\'); }, true);</script>')
                        encoded = html.encode('utf-8')
                        self.send_response(200)
                        self.send_header('Content-Type', 'text/html; charset=utf-8')
                        self.send_header('Content-Length', str(len(encoded)))
...
<body>
    ...
    <video id="player" controls playsinline poster="{preview_url}">
...
"""
                ...
                if url_param:
                    ...
                    try:
                        r = PROXY_POOL.request(
                            'GET',
                            target_url,
                            headers=req_headers,
                            preload_content=False,
                            timeout=urllib3.util.Timeout(connect=4.0, read=45.0)
                        )
                        ...
                        # Normal browser disconnect when seeking/pausing
                        safe_print(f"[Proxy] Browser disconnected during playback (normal seek/abort): {conn_err}")
                        if 'r' in locals():
                            try: r.release_conn()
                            except: pass
                        return
                    except Exception as e:
                        safe_print(f"Proxy Error for {target_url}: {e}")
                        if 'r' in locals():
                            try: r.release_conn()
                            except: pass
                        self.send_response(302)
...
def start_proxy_server():
    global PROXY_SERVER
    server = ThreadingHTTPServer(('127.0.0.1', proxy_port), VideoProxyHandler)
    PROXY_SERVER = server
...
def fetch_metadata_info(ccode, api_instance=None, priority=0, only_cache=False):
    if not ccode:
    ...
    if only_cache:
        if api_instance and hasattr(api_instance, 'data'):
            for item in api_instance.data:
                if clean_code(item.get("code", "")).upper() == ccode.upper():
                    return {
        ...
        return {"actress": "未知", "title": None, "japanese_title": None, "date": ""}
    ...
    try:
        ...
        if jable_info and jable_info.get("actress") and jable_info["actress"] != "未知":
            res = {"actress": jable_info["actress"], "title": jable_info["title"], "japanese_title": None, "date": date}
    ...
    if is_non_std:
        res = {"actress": actress, "title": title, "japanese_title": None, "date": date}
        AVWIKI_CACHE[ccode_lower] = res
        return res

    # === AV-Wiki Fetch (urllib first for speed, Playwright as upgrade) ===
    def _parse_avwiki_title_tag(title_text, ccode_upper):
        ...
        if found_actress != "未知":
            ...
            m_title = re.search(rf'{re.escape(ccode_upper)}：(.+?)阻海に出てるとかそういう表記', title_text)
    ...
    global AVWIKI_FAILED, JAVBUS_FAILED
    avwiki_url = f"https://av-wiki.net/{ccode_lower}/"
    avwiki_html = None
    
    if not AVWIKI_FAILED:
        # Try urllib first (fast, no overhead)
        try:
            ...
            _req = urllib.request.Request(avwiki_url, headers=_headers)
            with urllib.request.urlopen(_req, timeout=3) as _r:
                avwiki_html = _r.read().decode('utf-8', errors='ignore')
        except Exception:
            pass
        
        # Playwright fallback if urllib failed
        if not avwiki_html and api_instance and api_instance.browser_context:
            try:
                avwiki_html = api_instance.playwright_fetch_html(avwiki_url, timeout_ms=4000)
            except Exception as e:
                safe_print(f"Playwright av-wiki fetch failed: {e}")
                
        if not avwiki_html:
            safe_print(f"AV-Wiki: Unreachable for {ccode}. Setting AVWIKI_FAILED to True to bypass future checks.")
            AVWIKI_FAILED = True

    if avwiki_html:
        try:
            m_title_tag = re.search(r'<title>(.*?)</title>', avwiki_html, re.IGNORECASE | re.DOTALL)
            if m_title_tag:
                title_text = m_title_tag.group(1)
                found_actress, found_jp_title = _parse_avwiki_title_tag(title_text, ccode.upper())
                if found_actress != "未知":
                    actress = found_actress
                if found_jp_title:
                    japanese_title = found_jp_title
                    title = found_jp_title
            
            # Fallback to HTML body table parsing if actress name is still unknown
            if actress == "未知":
                m_body_actresses = re.search(r'AV女優名\s*：\s*(.*?)</li>', avwiki_html, re.DOTALL)
                if m_body_actresses:
                    actresses = re.findall(r'<a[^>]*>([^<]+)</a>', m_body_actresses.group(1))
                    if actresses:
                        actress = ", ".join(a.strip() for a in actresses)
            
            if actress != "未知" or japanese_title:
                res = {"actress": actress, "title": title, "japanese_title": japanese_title, "date": date}
                AVWIKI_CACHE[ccode_lower] = res
                return res
        except Exception as e:
            safe_print(f"av-wiki parse error for {ccode}: {e}")

    # === MissAV Fetch ===
    missav_url = f"https://missav.ws/zh/{ccode_lower}"
    missav_html = None
    try:
        _headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        ...
        _req = urllib.request.Request(missav_url, headers=_headers)
        with urllib.request.urlopen(_req, timeout=3) as _r:
            missav_html = _r.read().decode('utf-8', errors='ignore')
    except Exception:
        pass
        
    if not missav_html and api_instance and api_instance.browser_context:
        try:
            missav_html = api_instance.playwright_fetch_html(missav_url, timeout_ms=4000)
        except Exception as e:
            safe_print(f"Playwright MissAV fetch failed: {e}")
            
    if missav_html:
        try:
            m_title_tag = re.search(r'<meta\s+property=["\']og:title["\']\s+content=["\']([^"\']+)["\']', missav_html, re.IGNORECASE)
            if not m_title_tag:
                m_title_tag = re.search(r'<title>(.*?)</title>', missav_html, re.IGNORECASE | re.DOTALL)
            
            detail_title = None
            if m_title_tag:
                import html as html_module
                detail_title = html_module.unescape(m_title_tag.group(1).strip())
                
            actress_matches = re.findall(r'href="[^"]*actresses/([^"]+)"[^>]*>([^<]+)</a>', missav_html)
            missav_actresses = []
            if actress_matches:
                raw_names = list(set(name.strip() for url_part, name in actress_matches if name.strip()))
                stop_words = ["排行", "排名", "女優排行", "名單", "名册", "榜", "202", "199", "198", "actresses", "stars", "models"]
                for name in raw_names:
                    if not any(sw in name for sw in stop_words) and name not in ["女優", "女演員", "女优", "女演员", "Actresses", "Models", "モデル"]:
                        missav_actresses.append(name)
                        
            missav_actress = ", ".join(missav_actresses) if missav_actresses else "未知"
            
            if detail_title:
                clean_t = detail_title
                clean_t = re.sub(re.escape(ccode), '', clean_t, flags=re.IGNORECASE).strip()
                clean_t = re.sub(r'^(hd|1080p|720p|高清|字幕|中字|無碼破解|一般版)\s*', '', clean_t, flags=re.IGNORECASE).strip()
                clean_t = re.sub(r'^[\[【\(（].*?[\]】\)）]', '', clean_t).strip()
                clean_t = re.sub(r'\s*-\s*MissAV.*$', '', clean_t, flags=re.IGNORECASE).strip()
                title = clean_t
                japanese_title = clean_t
                
            if missav_actress != "未知":
                actress = missav_actress
            ...
            safe_print(f"MissAV parse error for {ccode}: {e}")

    cover = ""
    if not JAVBUS_FAILED and api_instance and api_instance.browser_context:
        try:
            javbus_url = f"https://www.javbus.com/{ccode.upper()}"
            html = api_instance.playwright_fetch_html(javbus_url, timeout_ms=6000)
            if html:
                if "Age Verification" in html or "所在地區年齡檢測" in html or "你是否已經成年" in html:
                    safe_print(f"JavBus: Age gate blocked or CAPTCHA driving exam triggered for {ccode}. Setting JAVBUS_FAILED to True.")
                    JAVBUS_FAILED = True
                else:
                    ...
                    m_cover = re.search(r'<a[^>]+class=["\']bigImage["\'][^>]+href=["\']([^"\']+)["\']', html)
                    if m_cover:
                        cover = m_cover.group(1).strip()
                        
                    if actress != "未知" or title or cover:
                        res = {"actress": actress, "title": title, "japanese_title": japanese_title, "date": date, "cover": cover}
                        AVWIKI_CACHE[ccode_lower] = res
                        return res
        except Exception as e:
    ...
    res = {"actress": actress, "title": title, "japanese_title": japanese_title, "date": date, "cover": cover}
    # Only cache if we succeeded in finding the actress, or if Playwright is initialized (meaning a complete check was done)
    if (actress != "未知" or japanese_title) or (api_instance and api_instance.browser_context):
        AVWIKI_CACHE[ccode_lower] = res
    return res

def parse_actress_from_title(title):
    """廢除從片名猜測女優名的邏輯，統一回傳未知，讓背景爬蟲進行精確查詢。"""
    return "未知"
...
class Api:
    def __init__(self):
        ...
        self.img_cache_dir = os.path.join(os.path.dirname(self.data_path), 'favorites_images')
        ...
        def try_repair_json(file_path):
            ...
            try:
                ...
                safe_print(f"Repair: Failed to read {file_path}: {e}")
                return None
            ...
            if content.startswith('[') and not content.endswith(']'):
                safe_print(f"Repair: File {file_path} seems truncated. Attempting to repair...")
                lines = content.splitlines()
                # 逐步去掉最後幾行測試
                for drop_count in range(0, min(50, len(lines))):
                    test_lines = lines if drop_count == 0 else lines[:-drop_count]
                    ...
                    try:
                        ...
                        safe_print(f"Repair: Successfully repaired {file_path} by dropping {drop_count} lines! Loaded {len(data)} items.")
                        return data
                    except Exception:
                        continue
                
                # 逐字元截斷測試
                for idx in range(len(content), max(0, len(content) - 2000), -1):
                    test_str = content[:idx].strip()
                    if test_str.endswith(','):
                        test_str = test_str[:-1].strip()
                    test_str += "\n]"
                    try:
                        data = json.loads(test_str)
                        safe_print(f"Repair: Successfully repaired {file_path} by character slicing! Loaded {len(data)} items.")
                        return data
                    except Exception:
                        continue
            return None
        ...
        candidates = [
            ...
            os.path.join(data_dir, 'Favorites.json.tmp'),
            os.path.join(data_dir, 'Favorites.json.corrupted.bak'),
        ]
        
        # 開發環境下，也把 main/ 目錄下的可能檔案當作備份候選
        if not hasattr(sys, 'frozen'):
            base_dir = os.path.dirname(os.path.abspath(__file__))
            if os.path.abspath(data_dir) != os.path.abspath(base_dir):
                candidates.extend([
                    os.path.join(base_dir, 'Favorites.json'),
                    os.path.join(base_dir, 'Favorites.json.tmp'),
                    os.path.join(base_dir, 'Favorites.json.corrupted.bak')
                ])
            ...
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(val, list):
                        loaded_options.append((val, path, 'normal'))
                        safe_print(f"Favorites Loader: Successfully read {path} directly. Len={len(val)}")
            except Exception:
                pass
                
            # 嘗試修復讀取
            try:
                rep = try_repair_json(path)
                if rep is not None and isinstance(rep, list):
                    loaded_options.append((rep, path, 'repaired'))
            except Exception:
                pass
        ...
        for val, path, status in loaded_options:
            if len(val) > len(best_data):
                best_data = val
                chosen_path = path
        ...
        if best_data:
            self.data = best_data
            safe_print(f"Favorites Loader: Loaded {len(self.data)} items from {chosen_path} ({chosen_status})")
            
            need_rewrite = (chosen_status == 'repaired') or (os.path.abspath(chosen_path) != os.path.abspath(self.data_path))
            if need_rewrite:
                safe_print(f"Favorites Loader: Active file {self.data_path} needs rewrite/fix. Saving current restored favorites...")
                self.save_videos(self.data)
        else:
            self.data = []
            safe_print("Favorites Loader: No valid favorites data found. Starting with empty list.")
        
        self.sync_favorites_images_cache()
    ...
    def get_videos(self):
        ...
        for item in self.data:
            copied = dict(item)
            code = item.get('code')
            orig_img = item.get('img', '')
            if code and orig_img:
                encoded_fallback = urllib.parse.quote(orig_img)
                copied['img'] = f"http://127.0.0.1:{proxy_port}/local_image?code={code}&fallback={encoded_fallback}"
    ...
    def request_actress_async(self, ccode):
        # Check cache first — return immediately if already available
        ccode_lower = ccode.lower().strip()
        if ccode_lower in AVWIKI_CACHE:
            return True
        def background_fetch():
            # Wait for browser context to be ready, but with shorter intervals
            for _ in range(20):
                if self.browser_context or sys.is_finalizing():
                    break
                time.sleep(0.3)
    ...
    def save_videos(self, new_data):
        self.data = new_data
        try:
            paths_to_save = [self.data_path]
            # In development mode, also save to main/Favorites.json if self.data_path is different
            if not hasattr(sys, 'frozen'):
                base_dir = os.path.dirname(os.path.abspath(__file__))
                dev_path = os.path.join(base_dir, 'Favorites.json')
                if os.path.abspath(dev_path) != os.path.abspath(self.data_path):
                    paths_to_save.append(dev_path)
                
                # Also save to main/dist/JApp/Favorites.json if it exists and is different
                dist_japp_path = os.path.join(base_dir, 'dist', 'JApp', 'Favorites.json')
                if os.path.exists(os.path.dirname(dist_japp_path)) and os.path.abspath(dist_japp_path) != os.path.abspath(self.data_path) and os.path.abspath(dist_japp_path) != os.path.abspath(dev_path):
                    paths_to_save.append(dist_japp_path)
    ...
    def download_image_to_cache(self, code, img_url):
        if not img_url: return
        cache_path = os.path.join(self.img_cache_dir, code)
        ...
        if 'tktube' in img_url or 'tkcdns' in img_url:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                with open(cache_path, 'wb') as f:
                    f.write(data)
            safe_print(f"[ImgCache] Downloaded cache for {code}")
        except Exception as e:
            safe_print(f"[ImgCache] Failed to download {img_url} for {code}: {e}")

    def sync_favorites_images_cache(self):
        try:
            os.makedirs(self.img_cache_dir, exist_ok=True)
            active_codes = set()
            code_to_url = {}
            for item in self.data:
                ...
                if code:
                    ...
                    code_to_url[code] = item.get('img')
                    
            if os.path.exists(self.img_cache_dir):
                for file in os.listdir(self.img_cache_dir):
                    file_path = os.path.join(self.img_cache_dir, file)
                    if os.path.isfile(file_path) and file not in active_codes:
                        try:
                            os.remove(file_path)
                            safe_print(f"[ImgCache] Removed obsolete cache file for {file}")
                        except Exception as e:
                            pass
                        
            def download_worker():
                for code in active_codes:
                    if sys.is_finalizing(): break
                    cache_path = os.path.join(self.img_cache_dir, code)
                    if not os.path.exists(cache_path) and code_to_url.get(code):
                        self.download_image_to_cache(code, code_to_url[code])
                        time.sleep(0.3)
                        
            threading.Thread(target=download_worker, daemon=True).start()
        except Exception as e:
            safe_print(f"[ImgCache] sync error: {e}")
    ...
    def fetch_video_info(self, raw_code):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(providers)) as executor:
            ...
            priority_list = ["tcav", "supjav", "tktube", "javhdporn"] if is_tcav_priority else ["tktube", "tcav", "javhdporn", "supjav"]
            ...
            if best_res:
                ...
                resolved_jp_title = (meta_info.get("japanese_title") if meta_info else None) or ""
                
                # Robust fallback: if actress name is still unknown or title is missing, fetch from the working detail page
                if (final_actress == "未知" or not resolved_jp_title) and r.get("url"):
                    success_detail, _, detail_actress, detail_title = fetch_url(r["url"])
                    if success_detail:
                        if final_actress == "未知" and detail_actress != "未知":
                            final_actress = detail_actress
                        if not resolved_jp_title and detail_title:
                            clean_t = detail_title
                            clean_t = re.sub(re.escape(ccode), '', clean_t, flags=re.IGNORECASE).strip()
                            clean_t = re.sub(r'^(hd|1080p|720p|高清|字幕|中字|無碼破解|一般版)\s*', '', clean_t, flags=re.IGNORECASE).strip()
                            clean_t = re.sub(r'^[\[【\(（].*?[\]】\)）]', '', clean_t).strip()
                            clean_t = re.sub(r'\s*-\s*(TKTube|TCAV).*$', '', clean_t, flags=re.IGNORECASE).strip()
                            clean_t = re.sub(r'^[a-zA-Z0-9\-]+\s*', '', clean_t).strip()
                            resolved_jp_title = clean_t
                ...
                return {
                    ...
                    "japanese_title": resolved_jp_title
                }
        ...
        if ccode and re.search(r'[A-Z0-9]+-[0-9]+', ccode):
            ...
            fallback_cover = (meta_info.get("cover") if meta_info else None) or "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=500&auto=format&fit=crop"
            ...
            return {
                ...
                "upload_date": resolved_date,
                "relative_time": f"加入日期: {resolved_date}" if resolved_date else "",
                "japanese_title": (meta_info.get("japanese_title") if meta_info else None) or ""
            }
    ...
    def enrich_videos_with_metadata(self, videos):
        ...
        def enrich_video(v):
            ccode = v.get("code")
            if ccode:
                meta_info = fetch_metadata_info(ccode, self, only_cache=True)
                if meta_info:
                    if meta_info.get("actress") and meta_info["actress"] != "未知":
                        v["actress"] = meta_info["actress"]
                    if meta_info.get("japanese_title"):
                        v["japanese_title"] = meta_info["japanese_title"]
    ...
    def search_all_platforms(self, query):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            query_is_actress = is_valid_actress_name(query)
            for vlist in platform_order:
                ...
                for v in vlist:
                    ...
                    if query_is_actress:
                        current_act = v.get("actress", "")
                        if not current_act or current_act in ("未知", "未知女優", "None", "") or not is_valid_actress_name(current_act):
                            v["actress"] = query.strip()
...
<body>
    ...
    def heal_favorites_background(self):
        ...
        for idx, item in enumerate(self.data):
            ...
            url = item.get('url', '')
            is_non_tktube = url and ('tktube.com' not in url) and ('tcav.85xvideo.com' not in url) and ('missav.ws' not in url)
            if (item.get('code') != ccode.upper()) or (not url) or (not item.get('found')) or is_invalid_actress(item.get('actress')) or is_non_tktube:
            ...
            for future in concurrent.futures.as_completed(future_to_idx):
                ...
                try:
                    ...
                    if res and res.get('found'):
                        ...
                        final_jp_title = self.data[idx].get('japanese_title', '')
                        if meta_info:
                            if meta_info.get("actress") and meta_info["actress"] != "未知":
                                final_actress = meta_info["actress"]
                            if meta_info.get("japanese_title"):
                                final_jp_title = meta_info["japanese_title"]
...
if __name__ == '__main__':
    ...
    window = webview.create_window(window_title, html_path, js_api=api, width=1350, height=920)
    api._window = window
